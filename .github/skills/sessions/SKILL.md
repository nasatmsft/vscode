...
- **Cooldowns as lifecycle guards**: Do not use timestamp cooldowns to prevent restart/retry loops in session-switch code. Cooldowns suppress legitimate rapid state changes; instead, model the actual operation with deterministic state such as an in-flight set, an observable transition, or the awaited operation's completion.
